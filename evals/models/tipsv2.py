...
VARIANT_MAP = {
    ...
    "B": (vit_base, 768, 12),
    "L": (vit_large, 1024, 24),
    "So400m": (vit_so400m, 1152, 27),
    "g": (vit_giant2, 1536, 40),
}

CHECKPOINT_URLS = {
    "S":  "https://storage.googleapis.com/tips_data/v2_0/checkpoints/pytorch/tips_v2_oss_s14_vision.npz",
    "B":  "https://storage.googleapis.com/tips_data/v2_0/checkpoints/pytorch/tips_v2_oss_b14_vision.npz",
    "L":  "https://storage.googleapis.com/tips_data/v2_0/checkpoints/pytorch/tips_v2_oss_l14_vision.npz",
    "So400m": "https://storage.googleapis.com/tips_data/v2_0/checkpoints/pytorch/tips_v2_oss_so14_vision.npz",
    "g":  "https://storage.googleapis.com/tips_data/v2_0/checkpoints/pytorch/tips_v2_oss_g14_vision.npz",
}

class TIPS(nn.Module):
  def __init__(self, variant="L", img_size=448, output="dense-cls",
                 layer=-1, return_multilayer=False, checkpoint_path=None):
    super().__init__()

    assert variant in VARIANT_MAP, f"Unkniwn variant: {variant}"
    vit_fn, embed_dim, depth = VARIANT_MAP[variant]
    self.checkpoint_name = f"tipsv2_{variant.lower()}14"
    self.patch_size = 14
    self.output = output
    ...
    if checkpoint_path is None:
        self._download_and_load(CHECKPOINT_URLS[variant])
    else:
        self._load_npz(checkpoint_path)
  ...
  def forward(self, images):
    ...
    result = self.vit.get_intermediate_layers(
      images, n=self.multilayers, reshape=True, return_class_token=True, norm=True
    )
    spatial, cls_tokens = zip(*result)
    outputs = []
    for sp, cl in zip(spatial, cls_tokens):
        sp_flat = sp.flatten(2).transpose(1, 2)  # (B, N, C)
        out = tokens_to_output(self.output, sp_flat, cl, (h, w))
        outputs.append(out)

    return outputs[0] if len(outputs) == 1 else outputs

  def _load_npz(self, path):
      ckpt = dict(np.load(path, allow_pickle=False))
      for k in ckpt:
          ckpt[k] = torch.tensor(ckpt[k])
      self.vit.load_state_dict(ckpt)

  def _download_and_load(self, url):
      import os
      cache_dir = os.path.expanduser("~/.cache/tips_checkpoints")
